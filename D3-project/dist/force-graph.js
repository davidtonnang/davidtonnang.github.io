// Version 1.43.1 force-graph - undefined
(function (global, factory) {
  ...
  **/
  ...
  const autoColorScale = ordinal(schemePaired);
  ...
  const updDataPhotons = (_, state) => {
    if (!state.isShadow) {
      ...
      const linkParticlesAccessor = index$2(state.linkDirectionalParticles);
  ...
  var CanvasForceGraph = index$3({
    ...
    methods: {
      ...
      tickFrame: function (state) {
        ...
        function paintNodes() {
          const getVisibility = index$2(state.nodeVisibility);
          const getVal = index$2(state.nodeVal);
          const getColor = index$2(state.nodeColor);
          const getNodeCanvasObjectMode = index$2(state.nodeCanvasObjectMode);
          const ctx = state.ctx;
        ...
        function paintLinks() {
          const getVisibility = index$2(state.linkVisibility);
          const getColor = index$2(state.linkColor);
          const getWidth = index$2(state.linkWidth);
          const getLineDash = index$2(state.linkLineDash);
          const getCurvature = index$2(state.linkCurvature);
          const getLinkCanvasObjectMode = index$2(state.linkCanvasObjectMode);
          const ctx = state.ctx;
          ...
          const linksPerColor = index(defaultPaintLinks, [getColor, getWidth, getLineDash]);
          ctx.save();
        ...
        function paintArrows() {
          ...
          const getLength = index$2(state.linkDirectionalArrowLength);
          const getRelPos = index$2(state.linkDirectionalArrowRelPos);
          const getVisibility = index$2(state.linkVisibility);
          const getColor = index$2(state.linkDirectionalArrowColor || state.linkColor);
          const getNodeVal = index$2(state.nodeVal);
          const ctx = state.ctx;
          ctx.save();
        ...
        function paintPhotons() {
          const getNumPhotons = index$2(state.linkDirectionalParticles);
          const getSpeed = index$2(state.linkDirectionalParticleSpeed);
          const getDiameter = index$2(state.linkDirectionalParticleWidth);
          const getVisibility = index$2(state.linkVisibility);
          const getColor = index$2(state.linkDirectionalParticleColor || state.linkColor);
          const ctx = state.ctx;
          ctx.save();
          state.graphData.links.filter(getVisibility).forEach(link => {
    ...
    update(state) {
      ...
      state.forceLayout.force('dagRadial', ['radialin', 'radialout'].indexOf(state.dagMode) !== -1 ? d3ForceRadial(node => {
  ...
  function adjustCanvasSize(state) {
    if (state.canvas) {
      ...
      const k = transform(state.canvas).k;
  ...
  var forceGraph = index$3({
    ...
    methods: {
      graph2ScreenCoords: function (state, x, y) {
        const t = transform(state.canvas);
        return {
          x: x * t.k + t.x,
          y: y * t.k + t.y
        };
        ...
        if (k !== undefined) {
          if (!transitionDuration) {
            ...
            new TWEEN$1.Tween({
              ...
              k
            }, transitionDuration).easing(TWEEN$1.Easing.Quadratic.Out).onUpdate(({
              k
            }) => setZoom(k)).start();
          }
          return this;
      ...
      getGraphBbox: function (state, nodeFilter = () => true) {
        ...
        return !nodesPos.length ? null : {
          x: [min$1(nodesPos, node => node.x - node.r), max$1(nodesPos, node => node.x + node.r)],
          y: [min$1(nodesPos, node => node.y - node.r), max$1(nodesPos, node => node.y + node.r)]
        };
      },
    ...
    init: function (domNode, state) {
      ...
      d3Select(state.canvas).call(d3Drag().subject(() => {
      ...
      const refreshShadowCanvas = throttle(() => {
        ...
        state.shadowGraph.linkWidth(l => index$2(state.linkWidth)(l) + state.linkHoverPrecision);
      ...
      (this._animationCycle = function animate() {
        ...
        if (state.enablePointerInteraction) {
          ...
          if (obj !== state.hoverObj) {
            ...
            const tooltipContent = obj ? index$2(state[`${obj.type.toLowerCase()}Label`])(obj.d) || '' : '';
        ...
        if (doRedraw) {
          ...
          // Frame cycle
          const globalScale = transform(state.canvas).k;
